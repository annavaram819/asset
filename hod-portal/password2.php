...
$_SESSION['password']=$password;
      if($p1==$password)
      {
        ...
        $con=mysqli_connect("localhost","kishore","Kishore@016","login");
           $con->query("UPDATE hod set phone='$p2' where mail like'%$name%'");
           $sql=$con->query("SELECT phone from hod where mail like '%$name%'");
           $data=$sql->fetch_array();
           if($data[0]!=$password)
           {
            echo"<div class='no-course'>
            Changing password done suceessfully
            <br><br>
