  ...
  if($_SERVER["REQUEST_METHOD"] == "POST")
{
 ...
 {
  $query=$con->query("SELECT course_no,course_code from courses order by course_no DESC LIMIT 1");
  if(mysqli_num_rows($query)>0)
  {
    if($row=mysqli_fetch_assoc($query))
    {
      $value=$row['course_no'];
      $value2=substr($value,6,10);
      $value2=$value2 +1;
      $value=$year . sprintf('%02s',$value1).sprintf('%04s',$value2);
   }
  }
  else
  {
	...
	$query1=$con->query("INSERT INTO courses (course_no,course_code,subject) values ('$value','$cont[0]','$cont[1]')") ;
 }
 fclose($handle);
 echo"<br><br>
